...
{
	/*
	先畫出空的鏡子並計算其depth buffer與stencil buffer
	清空depth buffer再畫鏡射的PPAP避免被遮蓋
	鏡射的ppap只有透過相對應的鏡子才能看見
	再畫一次空的鏡子來算出depth buffer
	最後畫出會互相遮蔽的鏡子
	*/
	...
	glStencilMask(0xFF); // 打開遮罩，寫入 stencil buffer
	glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE);//不上色，純粹計算stencil buffer
	glDepthMask(GL_TRUE); // 打開遮罩，寫入 depth buffer
	glClear(GL_STENCIL_BUFFER_BIT); // 清空stencil buffer
	glClear(GL_DEPTH_BUFFER_BIT); // 清空depth buffer
	for (int i = 0; i <6; i++)
	{
		
		glStencilFunc(GL_ALWAYS, i+1, 0xFF); // 設定同時通過depth test與stencil test的值為i+1
										//畫出鏡子
		glPushMatrix();
	...
	glClear(GL_DEPTH_BUFFER_BIT); // 清空depth buffer
	glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);//上色，畫出PPAP
													//畫出鏡射物
	
	glStencilMask(0x00); // 關閉遮罩，不在寫入stencil buffer
	glDepthMask(GL_TRUE); // 打開遮罩，寫入 depth buffer
	
	for (int i = 0; i < 6; i++)
	{
		glStencilFunc(GL_EQUAL, i+1, 0xFF); // stencil value 為 i+1 表示透過正確的鏡子觀看
		//畫出 ppap	
		glPushMatrix();
		glTranslated(ppap_pos[i][0], ppap_pos[i][1], ppap_pos[i][2]);
		glScalef(ppap_scale[i][0], ppap_scale[i][1], ppap_scale[i][2]);
		glRotated(ppap_rotate_angle[i], mirror_rotate_axis[i][0], mirror_rotate_axis[i][1], mirror_rotate_axis[i][2]);
	...
	glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE);//不上色，純粹計算depth buffer
														//glDepthMask(GL_TRUE);
	...
	glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);//上色，畫出鏡子
